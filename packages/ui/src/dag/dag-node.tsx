...
function NodeHeader({ kind, label }: { kind: keyof typeof kindIcon; label: string }) {
  const iconName = kindIcon[kind]
  return (
    <div className="flex items-center gap-1.5">
      {iconName
        ? <Icon name={iconName} theme="auto" size={16} />
...
function WorkerNodeInner({ data, selected }: NodeProps<DagNode>) {
  const d = data as WorkerNodeData
  return (
    <NodeShell border={kindBorder.worker} selected={selected}>
      <NodeHeader kind="worker" label={d.label} />
      {d.model && (
      ...
      {d.status && d.status !== 'pending' && (
        <Badge status={d.status} label={d.status} dot className="self-start mt-0.5" />
      )}
    </NodeShell>
  )
}
...
function SupervisorNodeInner({ data, selected }: NodeProps<DagNode>) {
  const d = data as SupervisorNodeData
  return (
    <NodeShell border={kindBorder.supervisor} selected={selected}>
      <NodeHeader kind="supervisor" label={d.label} />
...
function LaneNodeInner({ data, selected }: NodeProps<DagNode>) {
  const d = data as LaneNodeData
  return (
    <NodeShell border={kindBorder.lane} selected={selected}>
      <NodeHeader kind="lane" label={d.label} />
...
function TriggerNodeInner({ data, selected }: NodeProps<DagNode>) {
  const d = data as TriggerNodeData
  return (
    <NodeShell border={kindBorder.trigger} selected={selected}>
      <NodeHeader kind="trigger" label={d.label} />
      {d.cronExpr && (
...
function BudgetNodeInner({ data, selected }: NodeProps<DagNode>) {
  const d = data as BudgetNodeData
  return (
    <NodeShell border={kindBorder.budget} selected={selected}>
      <NodeHeader kind="budget" label={d.label} />
...
function BarrierNodeInner({ data, selected }: NodeProps<DagNode>) {
  const d = data as BarrierNodeData
  return (
    <NodeShell border={kindBorder.barrier} selected={selected}>
      <NodeHeader kind="barrier" label={d.label} />
