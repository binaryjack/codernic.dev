...
async function handleOpenItem(
  ...
  switch (item.kind) {
    ...
    case 'provider': {
      const workspaceRoot = dirs.agentsDir.includes('.agencee')
        ? dirs.agentsDir.substring(0, dirs.agentsDir.indexOf('.agencee'))
        : '';
      const filePath = path.join(workspaceRoot, '.agencee', 'config', 'llms', item.id);
      const doc = await vscode.workspace.openTextDocument(filePath);
      await vscode.window.showTextDocument(doc);
      break;
    }
