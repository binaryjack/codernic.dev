...
RunDagIntent.prototype.handle = async function (
  ...
  if (!dagFile) {
    reply({
      ...
      payload: {
        chunk: `❌ DAG file not found: ${intent.spec || '(unspecified)'}\n\nTry: \`/run-dag <name>\` where name matches a file in \`.agencee/config/agents/\``,
        done: true,
      },
    });
    return true;
...
`;
  ...
  await runDagStream(
    ...
    async (event) => {
      ...
      } else if ((event as any).type === 'galileus:reset-state') {
        reply({
          type: 'galileus:state',
    ...
    mcpClientInstance,
    { history: enhancedHistory as any, token },
    channel,
  );
