...
export const createCodernicParticipant = function (
  ...
  const handleRequest = async function (
    ...
    try {
      ...
      const isMemoryCmd = rawCommand === 'read-project-memory' || rawCommand === 'write-project-memory';
      const mode: CodernicMode = isMemoryCmd ? 'ask' : ((rawCommand as CodernicMode) || 'ask');

      channel.appendLine(
        `[Codernic] Mode: ${mode}, Prompt: "${request.prompt.slice(0, 100)}${request.prompt.length > 100 ? '...' : ''}"`,
      );

      // Handle project memory commands via VS Code subcommand routing
      if (isMemoryCmd) {
        const cmdType = rawCommand === 'read-project-memory' ? 'read-project-memory' : 'write-project-memory';
        const intent = {
          type: cmdType as 'read-project-memory' | 'write-project-memory',
          spec: request.prompt,
          rawCommand: `/${rawCommand} ${request.prompt}`,
        };
        channel.appendLine(`[Codernic] Detected command via VS Code subcommand: ${intent.type}`);
      ...
      // Always search using the user's actual prompt for accurate RAG grounding
      ...
      if (codeContext) {
        ...
        channel.appendLine(
          `[Codernic] WARNING: No code context found — index may need to be built. Suggest running /index.`,
        );
      }
      ...
      if (
        ...
        try {
          ...
          if (activePlan === null) {
            ...
            const planResponse = await request.model.sendRequest(planMessages, {}, token);
            let planText = '';
      ...
      // Use the model the user selected in the Copilot Chat header
      const model = request.model;
      if (!model) {
        stream.markdown('⚠️ No language model available. Please select one in the Copilot Chat header.');
