...
export const validateRouteFiles = async (
  ...
  const configDir = path.join(workspaceRoot, '.agencee', 'config', 'llms');
  try {
            ...
            for (const modelId of modelIds) {
              ...
              if (!configuredModelIds.has(modelId)) {
                reply({
                  type: 'ac:chat-error',
                  payload: {
                    text: `Validation Warning in ${file} for purpose '${purpose}': Model '${modelId}' is not configured in any provider file (or is not logged into Copilot).`
                  }
                });
              }
            }
...
export async function resolveAdapter(
  ...
  if (llmId.startsWith('custom:')) {
    ...
    if (provider.type === 'openai-compatible' || provider.type === 'local-ollama') {
       if (context && !provider.parameters?.apiKey) {
...
export const streamChat = async (
  ...
  try {
    ...
    if (data.llmId.startsWith('custom:')) {
      ...
      if (!root) throw new Error('Workspace root required for custom models');
