...
const INTELLIGENCE_DIR = path.join('config', 'intelligence');
...
export async function loadManifest(workspaceRoot: string): Promise<AnalyseManifest> {
  const manifestPath = ConfigPaths.resolveFile(workspaceRoot, path.join(INTELLIGENCE_DIR, MANIFEST_FILE));
  if (manifestPath) {
    try {
...
export async function saveManifest(
  ...
  const dir = path.join(ConfigPaths.getPrimaryWritePath(workspaceRoot), INTELLIGENCE_DIR);
