...
import { GetRouteProfileHandler } from './handlers/get-route-profile.handler';
...
import { GetLocalModelsHandler, DeleteModelHandler, LocalModelsDiscoveryHandler, RunModelSearchHandler } from './handlers/local-models.handler';
...
import { GetAssetHandler } from './handlers/get-asset.handler';
import { SaveAssetHandler } from './handlers/save-asset.handler';
import { SendSchemaHandler } from './handlers/send-schema.handler';
import { TestConnectionHandler } from './handlers/test-connection.handler';
import { DaemonActionHandler } from './handlers/daemon-action.handler';
import { SessionIpcHandler } from './handlers/session-ipc.handler';
import { SaveLayoutHandler } from './handlers/save-layout.handler';
import { LoadLayoutsHandler } from './handlers/load-layouts.handler';
import { FactoryResetHandler } from './handlers/factory-reset.handler';
import { SyncAggregatorModelsHandler } from './handlers/sync-aggregator-models.handler';
import { GetModelBenchmarksHandler } from './handlers/get-model-benchmarks.handler';
import { GetCloudModelsHandler } from './handlers/get-cloud-models.handler';
import { AddCloudModelHandler } from './handlers/add-cloud-model.handler';
import { createOckhamCompressContextHandler, createOckhamSearchModelsHandler, createOckhamSyncCatalogHandler } from './handlers/ockham.handler';
...
export const createMessageRouter = function (): {
  ...
  router.register('codernic:get-route-profile', new (GetRouteProfileHandler as unknown as HandlerCtor)());
  ...
  router.register('codernic:get-local-models', new (GetLocalModelsHandler as unknown as HandlerCtor)());
  router.register('codernic:local-models-discovery', new (LocalModelsDiscoveryHandler as unknown as HandlerCtor)());
  router.register('codernic:run-model-search', new (RunModelSearchHandler as unknown as HandlerCtor)());
  router.register('codernic:delete-model', new (DeleteModelHandler as unknown as HandlerCtor)());
  ...
  const sessionEvents = ['codernic:get-sessions', 'codernic:load-session', 'codernic:rename-session', 'codernic:delete-session', 'codernic:rebuild-index', 'codernic:update-session-schema', 'codernic:update-session-config'];
  for (const event of sessionEvents) {
    router.register(event, new (SessionIpcHandler as unknown as new (type: string) => MessageHandler)(event));
  }

  router.register('codernic:get-asset', new (GetAssetHandler as unknown as HandlerCtor)());
  router.register('codernic:save-asset', new (SaveAssetHandler as unknown as HandlerCtor)());
  router.register('codernic:send-schema', new (SendSchemaHandler as unknown as HandlerCtor)());
  router.register('codernic:test-connection', new (TestConnectionHandler as unknown as HandlerCtor)());
  router.register('codernic:daemon-action', new (DaemonActionHandler as unknown as HandlerCtor)());


  router.register('codernic:save-layout', new (SaveLayoutHandler as unknown as HandlerCtor)());
  router.register('codernic:load-layouts', new (LoadLayoutsHandler as unknown as HandlerCtor)());
  router.register('codernic:factory-reset', new (FactoryResetHandler as unknown as HandlerCtor)());
  router.register('codernic:sync-aggregator-models', new (SyncAggregatorModelsHandler as unknown as HandlerCtor)());
  router.register('codernic:get-model-benchmarks', new (GetModelBenchmarksHandler as unknown as HandlerCtor)());
  router.register('codernic:add-cloud-model', new (AddCloudModelHandler as unknown as HandlerCtor)());
  router.register('codernic:ockham-compress', createOckhamCompressContextHandler() as unknown as MessageHandler);
  router.register('codernic:ockham-search-models', createOckhamSearchModelsHandler() as unknown as MessageHandler);
  router.register('codernic:ockham-sync-catalog', createOckhamSyncCatalogHandler() as unknown as MessageHandler);

  // Dummy handler for ready to avoid warnings
  router.register('codernic:ready', { handle: async () => {} } as unknown as MessageHandler);
