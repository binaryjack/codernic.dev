...
GenerateHandler.prototype.handle = async function (
  ...
  let { task, mode, llmId, contextFiles, erathosSchema } = payload as {
    task: string;
    mode: CodernicMode;
    llmId?: string;
    contextFiles?: any[];
  ...
  } else if (task.startsWith('/brainstorm ') || task === '/brainstorm') {
    mode = 'brainstorm';
    task = task.replace(/^\/brainstorm\s*/i, '').trim();
  ...
  try {
    ...
    let systemContext = `${systemInstructions}\n\n` + 
      (codeContext
        ? `# Codebase Context (Ragtime Engine)\n\n${codeContext}\n\n---\n\n`
        : '⚠️ No codebase context available. Recommend running /index command first.\n\n');

    if (erathosSchema) {
      // Inject Erathos structural context
      systemContext += `\n# Erathos Visual Architecture Schema\n\nThe user has provided a visual schema architecture designed in Erathos. Use this to guide your implementation plan if relevant.\n\n\`\`\`json\n${JSON.stringify(erathosSchema, null, 2)}\n\`\`\`\n\n---\n\n`;
    }
    ...
    try {
      ...
      const dagPath = path.join(ConfigPaths.getPrimaryWritePath(workspaceRoot), 'active-plan.dag.json');
      ...
      const fallbackDagPath = path.join(ConfigPaths.getPrimaryWritePath(workspaceRoot), 'fallback-plan.dag.json');
      try {
        const fallbackDag = {
