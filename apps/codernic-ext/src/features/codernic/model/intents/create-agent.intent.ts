...
CreateAgentIntent.prototype.handle = async function (
  ...
  const slug = spec
    ...
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-|-$/g, '');
  const agentsDir = path.join(ConfigPaths.getPrimaryWritePath(workspaceRoot), 'agents');
  ...
  try {
    ...
    reply({
      ...
      payload: {
        chunk: `⚠️ Agent already exists: \`Config folder/${slug}.agent.json\`\n\nDelete it first or choose a different name.`,
        done: true,
      },
    });
    return true;
  ...
  const agentJson = {
    $schema: remoteSchemasUrl ? `${remoteSchemasUrl}agents.form.schema.json` : `https://raw.githubusercontent.com/codernic-dev/ai-agencee/main/schemas/agents.form.schema.json`,
    name: spec,
