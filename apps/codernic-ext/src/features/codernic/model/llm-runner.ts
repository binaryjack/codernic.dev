...
export async function callLlm(
  ...
  try {
    ...
    if (llmId.startsWith('custom:')) {
      ...
      onLog(`[llm-runner] Resolved provider: ${providerId}, type: '${provider.type}', raw JSON: ${JSON.stringify(provider)}`);

      if (provider.type === 'openai-compatible' || provider.type === 'local-ollama' || provider.type?.startsWith('cloud-')) {
    ...
    onLog(`[llm-runner] Starting completion via adapter: ${adapter.id} (Class: ${adapter.constructor.name})`);
