...
export const INTELLIGENCE_DIR = path.join('config', 'intelligence');

export function resolveIntelligencePath(workspaceRoot: string, file: string): string | null {
  return ConfigPaths.resolveFile(workspaceRoot, path.join(INTELLIGENCE_DIR, file));
}

export function getIntelligenceWritePath(workspaceRoot: string, file: string): string {
  return path.join(ConfigPaths.getPrimaryWritePath(workspaceRoot), INTELLIGENCE_DIR, file);
}
