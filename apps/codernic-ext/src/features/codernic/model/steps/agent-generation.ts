...
import { resolveIntelligencePath, getIntelligenceWritePath, type AnalyseRunOptions } from '../analyse-runner';
...
export async function runAgentGeneration(
  ...
  const techPath = resolveIntelligencePath(opts.workspaceRoot, 'tech-registry.json');
  const convPath = resolveIntelligencePath(opts.workspaceRoot, 'conventions.json');
  ...
  for (const rule of result.rules ?? []) {
    const rulePath = getIntelligenceWritePath(opts.workspaceRoot, rule.filename);
