...
export async function runDagStream(
  ...
  if (opts.history && opts.history.length > 0) {
    ...
    const historyPath = path.join(writePath, '.chat_history.json');
    try {
    ...
    rl.on('line', (line) => {
      ...
      try {
        ...
        if (
          ...
          // Relay natively to Atomos Structura NodeJS MCP Server
          fetch(process.env.VITE_ERATHOS_MCP_URL || 'http://localhost:9743', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              jsonrpc: '2.0',
              method: 'atomos-structura/report-progress',
              params: {
                schema_id: 'codernic-execution-graph',
                node_id: (event as Record<string, unknown>).laneId,
                status: status,
                log_stream: (event as Record<string, unknown>).delta || '',
              },
              id: crypto.randomUUID()
            })
          }).catch((err: any) => {
            log.appendLine(`[DagRunner] Failed to report progress to Atomos MCP: ${err}`);
          });
        }
      } catch {
...
export function resolveDagFile(spec: string | undefined, workspaceRoot: string): string {
  if (!spec) {
    return ConfigPaths.resolveFile(workspaceRoot, path.join('config', 'agents', 'dag.json')) 
           || path.join(ConfigPaths.getPrimaryWritePath(workspaceRoot), 'config', 'agents', 'dag.json');
  }
