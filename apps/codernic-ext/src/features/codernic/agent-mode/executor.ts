...
async function findDagFile(dagDescription: string, workspaceRoot: string): Promise<string | null> {
  const dagsPaths = ConfigPaths.resolveDirectories(workspaceRoot, 'dags');
  ...
  for (const dagsPath of dagsPaths) {
    const directPath = path.join(dagsPath, `${kebabName}.dag.json`);
    try {
  ...
  const searchDags = async (dirs: string[]): Promise<string[]> => {
  ...
  const dagFiles = await searchDags(dagsPaths);
...
export async function executeDag(
  ...
  if (!dagFilePath) {
    ...
    const dags = await searchDags(dagsPaths);
