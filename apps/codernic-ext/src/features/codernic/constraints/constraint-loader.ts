...
export async function loadConstraintManifest(workspaceRoot: string): Promise<string | null> {
  const filePath = ConfigPaths.resolveFile(workspaceRoot, 'constraints.md');
...
export async function ensureConstraintManifest(
  ...
  try {
    ...
    const targetDir = ConfigPaths.getPrimaryWritePath(workspaceRoot);
    const targetPath = path.join(targetDir, 'constraints.md');
