...
const PLAN_SUBPATH = path.join('.codernic', 'session', 'current-plan.json');
...
describe('session-plan', () => {
  ...
  describe('loadSessionPlan', () => {
    it('returns null when .codernic/session/ directory does not exist', async () => {
      const result = await loadSessionPlan(tmpDir);
      expect(result).toBeNull();
    });

    it('returns null when plan file is absent but directory exists', async () => {
      await fs.mkdir(path.join(tmpDir, '.codernic', 'session'), { recursive: true });
      const result = await loadSessionPlan(tmpDir);
      expect(result).toBeNull();
    });
  ...
  describe('saveSessionPlan', () => {
    it('creates the full .codernic/session/ directory tree when missing', async () => {
      const plan = makePlan();
  ...
  describe('clearSessionPlan', () => {
    ...
    it('leaves the parent directory intact after clearing', async () => {
      ...
      const dirExists = await fs
        .access(path.join(tmpDir, '.codernic', 'session'))
        .then(() => true)
        .catch(() => false);
