...
// Shape of an *.agent.json file on disk (as found in .agencee/config/agents/)
...
export const resolveAgentsDir = (workspaceRoot: string): string =>
  path.join(workspaceRoot, '.agencee', 'config', 'agents');
    ...
    for (const file of files.filter((f) => f.endsWith(DAG_SUFFIX))) {
      try {
        ...
        if (isDagRecord(parsed)) {
          ...
          entries.push({ id, name: parsed.name, description: parsed.description });
        }
      } catch {
        // skip malformed / unreadable file
      }
