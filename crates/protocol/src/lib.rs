...
pub enum CommandRequest {
    ...
    UpdateSessionSchema { session_id: String, schema: serde_json::Value, project_root: PathBuf },
    GetAsset { asset_type: String, id: String, project_root: PathBuf },
    SaveAsset { asset_type: String, id: String, content: String, mode: Option<String>, project_root: PathBuf },
    ResolveToolApproval {
        request_id: String,
        approved: bool,
    },
    ...
    OckhamSyncCatalog { project_root: PathBuf },
    OckhamSearchModels {
        query: String,
        match_type: String,
        project_root: PathBuf,
...
impl InterprocessStream {
    pub async fn connect() -> std::io::Result<Self> {
        #[cfg(unix)]
        {
            let stream = tokio::net::UnixStream::connect("/tmp/codernic.sock").await?;
            Ok(Self::Unix(stream))
        }
        #[cfg(windows)]
        {
            let client = tokio::net::windows::named_pipe::ClientOptions::new()
                .open(r"\\.\pipe\codernic_pipe")?;
...
impl InterprocessListener {
    pub fn bind() -> std::io::Result<Self> {
        #[cfg(unix)]
        {
            let listener = tokio::net::UnixListener::bind("/tmp/codernic.sock")?;
            Ok(Self::Unix(listener))
        }
        #[cfg(windows)]
        {
            let path = std::ffi::OsString::from(r"\\.\pipe\codernic_pipe");
